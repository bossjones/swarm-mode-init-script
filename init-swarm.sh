...
certcopy(){
  ...
  docker-machine  ssh node1 touch /dockerdata/domain.lock
}
...
portainer) # global
    ...
    docker-machine  ssh node1 "sudo  -p /dockerdata/portainer"
    docker-machine  ssh node1 "sudo chown -R docker:docker /dockerdata"
...
portainer-ssl) # global
  ...
  docker-machine  ssh node1 "mkdir -p /dockerdata/portainer"
  docker-machine  ssh node1 "sudo chown -R docker:docker /dockerdata"
  docker-machine  ssh node1 docker service rm portainer > /dev/null
  docker-machine  ssh node1 "docker service create \
                           --name 'portainer' \
                           ...
                           --mount type=bind,src=/dockerdata/portainer,dst=/data \
                           --mount type=bind,src=/var/run/docker.sock,dst=/var/run/docker.sock \
                           --label 'traefik.frontend.rule=Host:$2;PathPrefixStrip:/portainer' \
...
traefik)
   ...
   for (( i=1; i<=$N; i++ ))
   do
      docker-machine  ssh node$i docker service rm traefik-ssl
      docker-machine  ssh node$i "docker network create --driver=overlay traefik-net"
      ...
      cp ./traefik.toml.orig ./traefik.toml
      sed 's/<DOMAIN>/'$3'/g' ./traefik.toml
...
traefik-ssl)
   ...
   do
      ...
      docker-machine  ssh node$i docker network create --driver=overlay traefik-net > /dev/null
      docker-machine  ssh node$i "sudo mkdir -p /dockerdata/traefik/log"
      docker-machine  ssh node$i "sudo chown -R docker:docker /dockerdata"
      cp ./traefik-ssl.toml.orig ./traefik-ssl.toml
      sed 's/<DOMAIN>/'$3'/g' ./traefik-ssl.toml
...
certgen)
   certgen  $2
   ;;
*) echo "Usage:  ./init-swarm.sh <command> <node number> <domain>
       ...
       ./init-swarm.sh portainer-ssl mydomain.lan
